...
class CEBottleneck(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None,
        ...
        super(CEBottleneck, self).__init__()
...
class DepthwiseSeparableASPPHead(ASPPHead):
    ...
    def __init__(self, c1_in_channels, c1_channels, **kwargs):
        ...
        self.aspp_modules = DepthwiseSeparableASPPModule(
            ...
            act_cfg=self.act_cfg)
        if c1_in_channels > 0:
            self.c1_CE = CEBottleneck(c1_in_channels, c1_channels)
        else:
            self.c1_SCConv = None
        self.c2_CE = CEBottleneck(self.c2_channels, self.c2_channels)
        self.c3_CE = CEBottleneck(self.c3_channels, self.c3_channels)
        self.c4_CE = CEBottleneck(self.c4_channels, self.c4_channels)

        self.bottleneck2 = ConvModule(
            self.channels + self.c2_channels//2,
            self.channels,
            3,
            padding=1,
            conv_cfg=self.conv_cfg,
        ...
        self.bottleneck3 = ConvModule(
            self.channels + self.c3_channels//2,
            self.channels,
            3,
            padding=1,
            conv_cfg=self.conv_cfg,
        ...
        self.bottleneck4 = ConvModule(
            self.channels + self.c4_channels//2,
            self.channels,
            3,
            padding=1,
            conv_cfg=self.conv_cfg,
    ...
    def forward(self, inputs):
        ...
        if self.c1_CE is not None:
            c1_output = self.c1_CE(inputs[0])
            output = resize(
                input=output,
                size=inputs[0].shape[2:],
                mode='bilinear',
